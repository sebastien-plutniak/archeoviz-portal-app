...
sites <- metadata[ ! metadata$site.name %in% c("Tournoisis"),]
      ...
      <a href=https://analytics.huma-num.fr/archeoviz/pt target=_blank>Portuguese</a>.
                  ...
                  <h2>Use cases</h2>
                    ...
                    <li>Use the “Search” field to search sites by
                                              ...
                                              span(
                                                `data-toggle` = "tooltip", `data-placement` = "bottom",
                                                title = "dates, lithic industry, animal bone, pottery, antler, lithic core, eggshell, millstone, microfauna, coprolites, bone industry, shell, human bone, metal objects, archaeological site, ceramic building material
",
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    leafletProxy("map")  %>% 
      clearMarkers()    %>%
      clearMarkerClusters()   %>%
      setView(lng = sites[row, ]$lon, lat = sites[row, ]$lat, zoom = 8)  %>%
  ...
  output$specific.plot <- plotly::renderPlotly({
    ...
    ggplotly(specific.plot, tooltip = c("visits", "date", "instance")) %>%
  ...
  output$generic.plot <- plotly::renderPlotly({
    ...
    ggplotly(generic.plot, tooltip = c("visits", "instance", "date")) %>%
      config(displaylogo = FALSE,
             modeBarButtonsToRemove = list(
               'sendDataToCloud',
               'select2d',
